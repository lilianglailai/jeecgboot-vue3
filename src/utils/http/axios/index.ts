...
const transform: AxiosTransform = {
    ...
    transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
        ...
        const { code, result, msg,message, extend, success } = data;
       let msgtitle=message||msg

        // 这里逻辑可以根据项目进行修改
        const hasSuccess = data && Reflect.has(data, 'code') && (code === ResultEnum.SUCCESS || code === 200);
        if (hasSuccess) {
            if (success && (msgtitle) && options.successMessageMode === 'success') {
                //信息成功提示
                createMessage.success(msgtitle);
            }
            return result;
        }
    ...
    responseInterceptorsCatch: (error: any) => {
        ...
        const msg: string = (response?.data?.message || response?.data?.msg) ?? '';
